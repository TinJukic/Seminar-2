...
def findNodeToVisitGA(vehicle: Vehicle, currentNodeIndex: int, data: dict, weights: []) -> dict:
    ...
    for i in range(data["numOfLocations"]):
        ...
        # ako je cvor vec obidjen, preskoci ga ili ako je cvor pocetni (depot) ili ako je to trenutni cvor
        if node.obisao or i == data["depotIndex"] or i == currentNodeIndex:
            continue

        distanceToTravel = calculateDistance(currentNode, node)
        nodeDemand = currentNode.getDemand()

        # ovdje treba ocijeniti rjesenje pomocu genetskog algoritma
        evaluationForNode = distanceToTravel * weights[0] + nodeDemand * weights[1]
        ...
        for j in range(i + 1, data["numOfLocations"], 1):
            nextNode: VRPClasses.Coordinates = data["coordinates"][j]
            nextNodeDistance = calculateDistance(nextNode, node)
            nextNodeDemand = nextNode.getDemand()

            nextNodeEvaluation = nextNodeDistance * weights[0] + nextNodeDemand * weights[1]

            if nextNode.obisao or j == data["depotIndex"] or j == currentNodeIndex:
                continue

            # if nextNodeDistance < distanceToTravel and nextNodeEvaluation < evaluationForNode:
            if nextNodeEvaluation < evaluationForNode:
                i = j - 1
                shouldReset = True
                break
            else:
...
def bestSolutionVRP(data: dict, model):
    ...
    while True:
        ...
        for i in range(len(vehicles)):
            ...
            nodeToVisitIndex = \
                findNodeToVisitNN(vehicle=vehicle, currentNodeIndex=currentNodeIndex, data=data, model=model)[
                    "minDistanceNodeIndex"]
